...
const isProd = process.env.QA_AUDIT_PROD === '1' || process.env.E2E_START_SERVERS === '0';
const frontend = process.env.E2E_FRONTEND_URL ?? (isProd ? 'https://shop-the-barber.vercel.app' : 'http://127.0.0.1:3000');

const env = {
  E2E_FRONTEND_URL: frontend,
  E2E_START_SERVERS: isProd ? '0' : (process.env.E2E_START_SERVERS ?? '1'),
  E2E_API_BASE_URL: process.env.E2E_API_BASE_URL ?? (isProd ? process.env.PRODUCTION_API_URL : 'http://127.0.0.1:3001'),
  JOURNEY_READONLY: isProd ? '1' : (process.env.JOURNEY_READONLY ?? '0'),
};
...
code |= run('npx', ['playwright', 'test', 'e2e/journeys', '--project=clerk-browser', '--workers=1'], env);
