...
test.afterAll(() => {
  ...
  writeFileSync(
    resolve(reportDir, 'ux-audit-mobile-playwright.json'),
...
for (const route of PUBLIC_UX_ROUTES.slice(0, 6)) {
  test(`${route.label} mobile smoke + a11y`, async ({ page }) => {
    ...
    record({ type: 'a11y', route: route.path, label: route.label, viewport: 'mobile', ...scan });
...
test('guest has no client bottom tab bar on Explore', async ({ page }) => {
  ...
  record({ type: 'layout', route: '/Explore', check: 'no-client-bottom-nav', pass: true });
});
