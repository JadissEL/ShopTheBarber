...
export default function SignIn() {
    ...
    const handleOAuthLogin = (provider) => {
        if (provider === 'google' && !oauthStatus.google) {
            toast.info('Google Sign-In is not configured', {
                description: 'Contact support to enable Google authentication.'
            });
            return;
        }
        if (provider === 'apple' && !oauthStatus.apple) {
            toast.info('Apple Sign-In is not configured', {
                description: 'Contact support to enable Apple authentication.'
            });
            return;
        }
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button 
                            type="button"
                            variant="outline" 
                            className="h-12 bg-card border-border hover:bg-muted rounded-xl text-muted-foreground hover:text-foreground transition-all"
