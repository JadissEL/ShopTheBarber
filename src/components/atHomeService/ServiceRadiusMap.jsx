...
const clientIcon = new L.DivIcon({
    ...
    html: `<div style="background:#7c3aed;width:14px;height:14px;border-radius:50%;border:3px solid white;box-shadow:0 0 0 3px rgba(124,58,237,0.35);"></div>`,
    iconSize: [14, 14],
    iconAnchor: [7, 7],
});
...
export default function ServiceRadiusMap({
    ...
    if (baseLatitude == null || baseLongitude == null) {
        return (
            <div
                className={`rounded-xl border border-dashed border-border bg-muted/30 flex items-center justify-center text-sm text-muted-foreground ${className}`}
                style={{ height }}
            >
