...
export default function ExploreBarberGrid({
  ...
  }, [viewType, sortBy, searchTerm, sortedBarbers.length, filteredShops.length, exploreMode]);
          ...
          <>
            ...
            {!barbersError &&
              ...
              visibleBarbers.map((barber, index) => {
                const hasPromo =
                  hasPlatformPromos || (!!barber.shop_id && dealSet.has(barber.shop_id));
                return (
            ...
            {!barbersError && !barbersFetching && sortedBarbers.length === 0 && barbersCount > 0 ? (
              <ExploreEmptyState
                icon={Search}
                title="No matches"
                message={pageConfig.emptyFilteredMessage}
            ...
            {!barbersError && !barbersFetching && barbersCount === 0 ? (
              <ExploreEmptyState
                ...
                message="We're growing in your area. Check back soon."
                actionLabel="Refresh"
                onAction={onRetryBarbers}
              />
            ) : null}
