...
export default function BookingOffersPanel({
  ...
  if (isLoading) {
    return (
      <div className=" border border-border bg-muted/30 p-4 animate-pulse">
  ...
  return (
    <div className={cn('space-y-4', compact ? 'mb-4' : 'mb-8')}>
      {promotions.length > 0 && (
      <div className={cn(stb.notice, 'border border-primary/20 bg-primary/5 p-4')}>
          ...
          <div className="flex flex-wrap gap-2">
            {promotions.map((p) => (
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-primary/30 bg-card hover:bg-primary/10 transition-colors text-left"
              >
      ...
      {matched_bundle && (
        <div className={cn(stb.noticeWarm, 'border border-primary/30 bg-primary/10 p-4 flex items-start gap-3')}>
          <Check className="w-5 h-5 text-primary shrink-0 mt-0.5" />
          <div>
            <p className={cn(stb.uiSubheading, 'text-success')}>{matched_bundle.bundle_name} applied</p>
            <p className={cn(stb.caption, 'text-success/80')}>
      ...
      {near_miss_bundles.length > 0 && (
        <div className="space-y-3">
          ...
          {near_miss_bundles.map((offer) => (
            <div
              key={offer.bundle_id}
              className="border border-primary/30 bg-primary/10 p-4 flex flex-col sm:flex-row sm:items-center gap-4"
            >
              <div className="flex-1 min-w-0">
                ...
                <p className={cn(stb.caption, 'text-success font-semibold mt-1')}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {available_bundles.map((offer) => (
              <div
                key={offer.bundle_id}
                className={cn(stb.panel, 'p-4 hover:border-primary/40 transition-colors')}
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  <div>
                    ...
                    <Badge className="ml-2 stb-chip stb-chip-active hover:bg-emerald-100">
